...
- **網頁介面**：連線管理、schema 樹狀瀏覽、資料表分頁／排序、多條件 `AND`／`OR` Filter Builder 與最近篩選歷史、可保存的欄位隱藏／寬度／順序／凍結設定、儲存格矩形選取與 TSV 複製／安全 staged 貼上、JSON／PostgreSQL array／大型文字完整內容 modal、binary／`bytea` 大小與 checksum 摘要、原始內容下載及 staged 檔案上傳、以 primary key 或非 NULL unique key 定位的安全 cell editing、資料列新增／Clone／刪除、dirty 標記與 staged changes（參數化 SQL 預覽、全部套用／回復、transaction rollback、原值與 row version 檢查）、大結果 SSE 串流（可取消）
