...
- **SQL 工作台**：多分頁編輯器（草稿自動保存）、執行選取／游標 statement／完整 script、`$n` query parameters、manual transaction（狀態、Commit、Rollback）、多結果分頁、目前／前次結果比對、PostgreSQL NOTICE/WARNING 訊息、執行中查詢可停止、PostgreSQL 格式化、schema-aware 自動補全、查詢歷史與已存查詢、結果匯出 CSV/TSV/JSON/Markdown
