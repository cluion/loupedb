...
- **網頁介面**：連線管理、schema 樹狀瀏覽、資料表分頁/排序/篩選、以 primary key 或非 NULL unique key 定位的安全 inline editing、資料列新增／Clone／刪除、dirty 標記與 staged changes（參數化 SQL 預覽、全部套用／回復、transaction rollback、原值與 row version 檢查）、大結果 SSE 串流（可取消）
