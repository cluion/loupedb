...
- **網頁介面**：連線管理、schema 樹狀瀏覽、資料表分頁/排序/篩選、以 primary key 或非 NULL unique key 定位的安全 inline editing、資料列新增／Clone／刪除（參數化 SQL 預覽、原值與 row version 檢查）、大結果 SSE 串流（可取消）
- **SQL 工作台**：多分頁編輯器（草稿自動保存）、執行選取／游標 statement／完整 script、`$n` query parameters、manual transaction（狀態、Commit、Rollback）、多結果分頁、目前／前次結果比對、PostgreSQL NOTICE/WARNING 訊息、執行中查詢可停止、PostgreSQL 格式化、schema/table/column/function 自動補全、查詢歷史、可收藏並以資料夾／標籤整理的已存查詢、結果匯出 CSV/TSV/JSON/Markdown
